...
def edit_profile(request, id):  # Use 'id' instead of 'pk'
    user_id = request.session.get('user_id')
    ...
    if request.method == 'POST':
        mydata = Supervise.objects.get(id=id)
...
def deletesupervisor(request,id):
    obj2=Supervise.objects.get(id=id)
    obj2.delete()
...
def editmanager(request, id):
    if request.method=="POST":
        ...
        RePassword=request.POST.get["ConfirmPassword"]
    ...
    return render(request,'view_manager')
...
def editrack(request, rack_id):
    ...
    try:
        ...
        if form.is_valid():
            form.save()
            return redirect('view_rack', id=myrack.foreign_key_fied.id)
        else:
            myrack = Racks.objects.get(id=rack_id)
